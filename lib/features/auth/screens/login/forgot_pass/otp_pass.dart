...
// Password Reset OTP entry bottom sheet
class PasswordResetOtpSheet extends ConsumerStatefulWidget {
  final String identifier;
  final VoidCallback onVerificationComplete;
  final String role;
  final String resetToken;

  const PasswordResetOtpSheet({
    super.key,
  ...
  @override
  ConsumerState<PasswordResetOtpSheet> createState() =>
      _PasswordResetOtpSheetState();
}
...
class _PasswordResetOtpSheetState extends ConsumerState<PasswordResetOtpSheet> {
  ...
  Future<void> _resendOTP() async {
    ...
    try {
      ...
      // Request password reset again to get a new token and OTP
      final newToken = await authRepository.requestPasswordReset(
        context,
        widget.identifier,
        widget.role,
      );
      ...
      if (newToken != null) {
        ...
        await prefs.setString('resetToken', newToken);
        ...
        showSnackBar(context: context, message: 'Failed to send OTP');
      }
    } catch (e) {
      showSnackBar(
  ...
  Future<void> _verifyOTP() async {
    ...
    if (otp.length == 6) {
      ...
      try {
        // We're using the existing OTP verification mechanism from AuthRepository
        final authRepository = AuthRepository();

        // In a real implementation, we would have an API call to verify the OTP for password reset
        // For now, we'll use the existing OTP verification method
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            "A 6-digit verification code has been sent to ${widget.identifier}. Please enter it below to verify your identity.",
            style: TextStyle(
              fontSize: 14,
              color: grey600,
            ),
